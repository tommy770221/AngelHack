...
public interface MemberMessageRepository extends CrudRepository<MemberMessage,String> {
    ...
    @Query("SELECT usr FROM MemberMessage usr  WHERE (usr.toEmail =:toEmail and usr.fromEmail=:fromEmail) or " +
            "(usr.fromEmail =:toEmail and usr.toEmail=:fromEmail) order by usr.createDate desc")
